...
const ListItems = ({refresh = 0}) => {
  ...
  const deleteItem = () => {
    dispatch(fetchStart());
    BankBranchService.deleteItem(item['uuid']).then(response => {
      handleRefreshGrid();
  ...
  const listItems = (props) => {
    ...
    setLoading(true);
    return BankBranchService.listItem(reqData).then(response => {
      setLoading(false);
      let data = [];
      let count = 0;
  ...
  const filterValue = [
    ...
    {name: 'branch_code', operator: 'startsWith', type: 'string', value: ''},
    {name: 'ifsc_code', operator: 'startsWith', type: 'string', value: ''},
    {name: 'contact_person', operator: 'startsWith', type: 'string', value: ''},
    {name: 'contact_number', operator: 'startsWith', type: 'string', value: ''},
    {name: 'branch_address', operator: 'startsWith', type: 'string', value: ''},
