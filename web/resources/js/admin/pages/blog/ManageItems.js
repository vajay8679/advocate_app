...
const ManageItems = ({entity_type = ''}) => {
  ...
  return (<>
    <AppComponentHeader
      title={<IntlMessages id="common.category.manage"/>}
      description={<IntlMessages id="common.category.description"/>}
      action={action()}
    />
    <AppAnimate animation="transition.slideUpIn" delay={200}>
      <AppGridContainer>
        <Grid item xs={12} md={12} lg={12}>
          <AppComponentCard
            title=''
            component={<ListItems refresh={refresh} entity_type={entity_type}/>}
          >
          </AppComponentCard>
        </Grid>
      </AppGridContainer>
    </AppAnimate>
    <AddItem isOpen={addItemModal} handleOpen={handleAddItemModal} entity_type={entity_type}/>
  </>);
}
