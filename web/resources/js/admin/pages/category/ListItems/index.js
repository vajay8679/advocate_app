...
const ListItems = ({refresh = 0, entity_type = ''}) => {
    ...
    const deleteItem = () => {
        dispatch(fetchStart());
        CategoryService.deleteItem(item['uuid']).then(response => {
            handleRefreshGrid();
    ...
    const changeStatus = (status) => {
        dispatch(fetchStart());
        CategoryService.updateItem(item['uuid'], {
    ...
    const handleUpdateStatusChange = (event, item)=>{
        ...
        dispatch(fetchStart());
        CategoryService.updateItem(item.uuid, data).then(response => {
            dispatch(fetchSuccess());
            dispatch(showMessage(response.message));
            handleRefreshGrid()
        }).catch(error => {
    ...
    const listItems = (props) => {
        ...
        setLoading(true);
        return CategoryService.listItem(reqData).then(response => {
            setLoading(false);
            let data = [];
            let count = 0;
    ...
    />
    <UpdateItem values={item} isOpen={updateItemModal} handleOpen={handleEditItemModal} entity_type={entity_type}/>
