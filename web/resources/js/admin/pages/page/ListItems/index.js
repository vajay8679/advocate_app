...
const ListItems = ({refresh = 0}) => {
    ...
    const deleteItem = () => {
        dispatch(fetchStart());
        PageService.deleteItem(item['uuid']).then(response => {
            handleRefreshGrid();
    ...
    const changeStatus = (status) => {
        dispatch(fetchStart());
        PageService.updateItem(item['uuid'], {
    ...
    const handleUpdateStatusChange = (event, item)=>{
        ...
        dispatch(fetchStart());
        PageService.updateItem(item.uuid, data).then(response => {
            dispatch(fetchSuccess());
            dispatch(showMessage(response.message));
            handleRefreshGrid()
        }).catch(error => {
    ...
    const listItems = (props) => {
        ...
        setLoading(true);
        return PageService.listItem(reqData).then(response => {
            setLoading(false);
            let data = [];
            let count = 0;
